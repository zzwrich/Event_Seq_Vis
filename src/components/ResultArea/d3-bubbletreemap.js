...
function createClusterBody(layerCluster, world) {
    ...
    layerCluster.nodes.forEach(function(circle) {
        ...
        let fixture = body.createFixture(planck.Circle(centerLocal, circle.r + circle.planckPadding,circle.parent.data.name), circleFD);
...
function Arc(x, y, startAngle, endAngle, radius, name) {
...
function contour(nodes, curvature) {
    ...
    nodes.forEach(function (node) {
        ...
        circles.push(new Circle(node.x, node.y, node.r + node.contourPadding, node.parent.data.name));
    });
...
function generateCircleArcs(outerCircleRing) {
    ...
    for (let i = 0; i < outerCircleRing.length; i++) {
        ...
        arcs.push(new Arc(circle.center.x, circle.center.y, arcStartAngle, arcEndAngle, circle.radius,circle.name));
    }
...
function generateTangentArcs(outerCircleRing, curvature) {
    ...
    for (let i = 0; i < outerCircleRing.length; i++) {
        ...
        arcs.push(new Arc(intersection.x, intersection.y, arcStartAngle, arcEndAngle, curvature,secondCircle.name));
    }
